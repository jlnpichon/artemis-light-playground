...
use artemis_playground::{
    common::{
        engine::*, pools::PoolResolver, provider::build_provider_ws, telemetry::Telemetry,
        tx::log_swap,
    },
    decoders::{decode_defi_tx, DefiEvent},
};

struct PoolResolverStrategy<P> {
...
#[async_trait]
impl<P: Provider + Clone> Strategy<Event, ()> for PoolResolverStrategy<P> {
    ...
    async fn process_event(&mut self, event: Event) -> Result<ActionStream<'_, ()>> {
        ...
        for event in events {
            if let DefiEvent::UniswapSwap(swap) = event {
                if let Some(meta) = self.resolver.resolve(&swap).await {
                    log_swap(&swap, &meta);
                }
            }
        }
...
async fn main() -> Result<()> {
    ...
    engine.add_strategy(Box::new(PoolResolverStrategy {
